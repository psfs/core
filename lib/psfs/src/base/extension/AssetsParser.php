...
class AssetsParser{
    ...
    {
        if(file_exists(BASE_DIR . $filename) && preg_match('/\.' . $this->type . '$/i', $filename)) $this->files[] = $filename;
        return $this;
    }
    ...
    {
        $base = BASE_DIR . DIRECTORY_SEPARATOR . "html" . DIRECTORY_SEPARATOR . "css" . DIRECTORY_SEPARATOR;
        ...
        if(!empty($this->files)) foreach($this->files as $file)
        {
            if(file_exists(BASE_DIR . $file))
            {
                ...
                if(!file_exists($base . $file_path) || filemtime($base . $file_path) < filemtime(BASE_DIR . $file))
                {
                        ...
                        while (!feof($handle)) {
                            ...
                            if(preg_match_all('#url\((.*?)\)#', $line, $urls, PREG_SET_ORDER))
                            {
                                foreach($urls as $source)
                                {
                                    ...
                                    $orig = realpath(dirname(BASE_DIR . $file) . DIRECTORY_SEPARATOR . $source_file);
                                    $orig_part = preg_split('/\/public\//i', $orig);
                                    $dest = BASE_DIR . DIRECTORY_SEPARATOR . 'html' . DIRECTORY_SEPARATOR . $orig_part[1];
                ...
                if($debug)
                {
                    $data = file_get_contents(BASE_DIR . $file);
    ...
    {
        $base = BASE_DIR . DIRECTORY_SEPARATOR . "html" . DIRECTORY_SEPARATOR . "js" . DIRECTORY_SEPARATOR;
        if(!file_exists($base)) @mkdir($base);
        $data = '';
        if(!empty($this->files)) foreach($this->files as $file)
        {
            ...
            if(file_exists(BASE_DIR . $file))
            {
                if($debug)
                {
                    ...
                    {
                        $js = file_get_contents(BASE_DIR . $file);
